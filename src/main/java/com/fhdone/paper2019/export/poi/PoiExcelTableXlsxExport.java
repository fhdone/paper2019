...
public class PoiExcelTableXlsxExport implements TableExport {

	private PoiDataFormatXlsxFactory dataformatFactory;
	...
	private void addRowToExl(ExcelRow er , XSSFWorkbook wb , XSSFSheet sheet , int rowNum ) throws Exception {
		if(dataformatFactory==null) {
			dataformatFactory = new PoiDataFormatXlsxFactory(wb);
		}
		String wcfStr = null;
		if(er.getDf()!=null) {
		...
		XSSFCellStyle hcs = dataformatFactory.getCellForamtMap().get(wcfStr);
		if(hcs==null) {
